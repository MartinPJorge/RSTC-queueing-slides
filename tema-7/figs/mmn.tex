...
% the circle
\node[draw,circle,minimum size=1.5cm,anchor=west] (se) at
    ($(wa.east)+(1,2)$) {$\mu$};
\node[draw,circle,minimum size=1.5cm,anchor=west] (se2) at
    ($(wa.east)+(1,.3)$) {$\mu$};
\node (dots) at ($(wa.east)+(1.7,-1)$) {$\vdots$};
\node[draw,circle,minimum size=1.5cm,anchor=west] (seN) at
    ($(wa.east)+(1,-2.3)$) {$\mu$};
